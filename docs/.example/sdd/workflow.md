...
- 最終チェック
  - 仕様書の確認
    - /docs/ui/配下の全仕様書を確認
    - /docs/features/配下の全仕様書を確認
    - /docs/schemas/配下の全仕様書を確認
    - 仕様書の完成度を評価
    - 仕様書の内容が正確か確認
    - 仕様書に不足がないか確認
  - テストコードの確認
  ...
  - 実装コードの確認
    ...
    - 実装コードに不足がないか確認
  - 過去PJとの照合
    - docs/.old/{日付}/backup/の過去PJコードを確認
    - 過去PJの機能が全て実装されているか確認
    - 過去PJの動作が全て再現されているか確認
    - 機能の後退がないか確認
  - 過去PJの仕様書とテストコードとの照合
    - docs/.old/{日付}/specification/の過去PJ仕様書を確認
    - docs/.old/{日付}/tests/の過去PJテストコードを確認
    - 過去PJの仕様書と現在の仕様書を照合
    - 過去PJのテストコードと現在のテストコードを照合
    - 過去PJの仕様・テストが全て反映されているか確認
  - テスト実行結果の確認
    - 全テスト通過を確認
    - テスト実行レポートを確認
    - テストの信頼性を評価
  - 完成度レポート作成
    - レポート作成: docs/.log/{日付}/FINAL_CHECK_REPORT.md
      - チェック結果サマリー
      - 仕様書の確認結果
        - 確認した仕様書の一覧
        - 仕様書の完成度評価
      ...
      - 完成度評価
        ...
        - 完成度が十分な場合の確認事項
      - 次のステップ
        - 完成度が低い場合: 修正項目の説明と修正手順
        - 完成度が十分な場合: 次の工程への移行
  - 完成度が低い場合: 修正 → 確認（上記の確認を繰り返す）
    - 不足や誤りを修正
    - 再度全項目を確認
    - 完成度レポートを更新
  - 完成度が十分な場合: 変更がされた箇所の説明と、完成度の報告
